...
impl MicrophoneStream {
    pub fn new(device_id: Option<String>) -> Result<Self> {
        ...
        // Try to find the requested device by name, fall back to default
        let device = if let Some(ref requested_name) = device_id {
            let mut found = None;
            if let Ok(devices) = host.input_devices() {
                for d in devices {
                    if let Ok(name) = d.name() {
                        if name == *requested_name || name.contains(requested_name.as_str()) {
                            println!("[Microphone] Matched device: {}", name);
                            found = Some(d);
                            break;
                        }
                    }
            ...
            found.or_else(|| {
                println!("[Microphone] Device '{}' not found, using default", requested_name);
