...
export type SttProviderId = 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'whisper-server';
...
export const STT_PROVIDERS: Record<SttProviderId, SttProviderConfig> = {
    ...
    'whisper-server': {
        id: 'whisper-server',
        name: 'Whisper Server (Self-hosted)',
        description: 'OpenAI-compatible self-hosted Whisper server (e.g. faster-whisper-server)',
        endpoint: '{serverUrl}/v1/audio/transcriptions',
        model: '',
        uploadType: 'multipart',
        authHeader: (apiKey: string) => apiKey ? { Authorization: `Bearer ${apiKey}` } : ({} as Record<string, string>),
        responseContentPath: 'text',
    },
};
