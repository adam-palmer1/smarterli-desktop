...
const App: React.FC = () => {
  ...
  // Check server connection on mount (only for launcher / default window)
  useEffect(() => {
    ...
    <div className="h-full min-h-0 w-full relative">
      <AnimatePresence>
        {showStartup ? (
          ...
          /* Auth check still in progress - show minimal loading state */
          <motion.div
            key="auth-loading"
            className="h-full w-full flex items-center justify-center bg-bg-primary"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <LoginScreen onConnected={() => setIsAuthenticated(true)} />
          </motion.div>
        ) : (
          <motion.div
            key="main"
